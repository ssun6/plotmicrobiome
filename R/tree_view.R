...
library(ggpubr)
library(ggtree)
tree_view <- function(taxa_table = NULL, metadata=NULL,fdrs=NULL,test_metadata=NULL,fdr_cutoff=0.1,node_size_breaks=c(0,0.01,0.05,0.5,5),taxa_removal="",palette_highlight=c("red","blue","orange","green")) {
  ...
  taxa_table=taxa_table[!grepl("__--__--__",rownames(taxa_table)),]

  names1=rownames(taxa_table)
  ln=sapply(strsplit(names1,"--"),length)
  names7=names1[ln==7]
  ...
  tab_p=t(t(taxa_table)/colSums(taxa_table))*100
  ...
  tab1_5=apply(taxa_table,1,function(i){length(which(i!=0))})/ncol(taxa_table)
  a=data.frame(cbind(rownames(taxa_table),rowMeans(tab1_3),tab1_5,tab1_4,fdrs,ln))
  ...
  if (any(dd$level<6)){
    ...
    dd2$group=droplevels(a$group[match(dd2$label,a$taxa)])
    dd2$level=a$level[match(dd2$label,a$taxa)]
    dd2$ext=6-dd2$level
    num_group=match(levels(dd2$group),names(table(map_s[[test_metadata]])))
    ...
    for (i in unique(dd2$level)){
      dd3=dd2[dd2$level==i,]
      num_group=match(levels(dd3$group),names(table(map_s[[test_metadata]])))
      ext1=(6-i)*0.6
      p = p + ggtree::geom_hilight(data=dd3,mapping=aes(fill=group, node=node_num),alpha=0.2,extend=ext1) +
    ...
    if (!taxa_removal==""){
      dd4=dd4[-which(dd4$letter1%in%taxa_removal),]
    }
    ...
    for (j in unique(dd4$level)){
      ...
      ext1=(6-j)*0.6+0.1
      for (i in 1:nrow(dd5)){
        p=p + ggtree::geom_cladelabel(node = dd5$node_num[i], label = dd5$letter1[i],barsize =0,angle = 'auto',horizontal = F,offset.text = ext1,hjust = "center", fontsize = 3)
      }
    }
    ...
    dd6$y=c(nrow(dd6):1)
    num_group=match(levels(dd6$group),names(table(map_s[[test_metadata]])))
  ...
  num_group=match(levels(dd$group),names(table(map_s[[test_metadata]])))
