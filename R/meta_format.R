...
meta_format=function(metadata=NULL,metadata_sep=",",meta_sample_name_col=0){
  if(metadata_sep==","){
    map=read.csv(file=metadata,header=T,check.names=FALSE,comment.char = "")
  }else{
    map=read.table(file=metadata,sep=metadata_sep,header=T,check.names=FALSE,comment.char = "")
  }
