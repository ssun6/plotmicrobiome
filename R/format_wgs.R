...
format_wgs <- function(taxa_file = NULL,sep="\t",method="metaphlan",normalization=T,reads_cutoff=0,rarefy=F,rarefy_num=1000) {
  ...
  if (method=="kraken"){
    ...
    if (!is.null(nrow(tab_all1))){
      tab_all=tab_all1
      if(!is.null(nrow(tab_all2))){
        tab_all=rbind(tab_all,tab_all2)
      }
      if(!is.null(nrow(tab_all3))){
        tab_all=rbind(tab_all,tab_all3)
      }
    }else if (!is.null(nrow(tab_all2))){
      tab_all=tab_all2
      if(!is.null(nrow(tab_all3))){
        tab_all=rbind(tab_all,tab_all3)
      }
    }else if (!is.null(nrow(tab_all3))){
      tab_all=tab_all2
    }else{
      stop("Not enough taxa!")
    }
