...
meta_corplot=function(taxa_table = NULL, metadata=NULL,test_metadata=NULL,col_metadata=NULL,page=1,log_norm=T,fdr_cutoff=0.1,cor_method="spearman",taxa_shown="",palette_group=c("red","blue","orange","green")){
  ...
  for (j in 1:nrow(cor_mat)){
    if (cor_mat[j,3]<fdr_cutoff){
      ...
      if(cor_mat[j,3]<0.001){
        ...
        wil_p=formatC(cor_mat[j,3], digits = 2)
      }
      ...
      gplots1[[k]]=g+annotate(geom="text", x=min(map1$i)+sd(map1$i)*2, y=max(map1$test_metadata)-sd(map1$test_metadata)*0.5, label=main1,color="black",size=3)
  ...
  if (length(gplots1)==0){
    stop("No Significant hits! Try to increase the FDR cutoff.")
  }
