...
#' @examples
#'
taxa_boxplot=function(taxa_table = NULL, metadata=NULL,test_metadata=NULL,fdrs=NULL,log_norm=T,cutoff=0.1,xlab_direction=1,palette_group=c("red","blue","orange","green"),page=1,taxa_shown=""){
  ...
  for (i in l1){
    ...
    boxplot(tab1n[i,]~metadata[,test_metadata],main=paste(tax_name1,"\nFDR =",wil_p),border=palette_group,xlab=test_metadata,ylab="normalized abundance",cex.main=0.8,las=xlab_direction)
